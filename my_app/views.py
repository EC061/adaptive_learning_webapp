...
import os
from .ML.ncdm_inference import predict, MODEL
...
def signup_view(request):
    if request.method == "POST":
        ...
        is_english_first_language = request.POST.get(
            "is_english_first_language"
        )  # New field for student info
        science_experience = request.POST.get(
            "science_experience"
        )  # New field for student info
...
def home_view(request):
    ...
    if not student.can_take_experimental_test:
        return render(
            request, "my_app/home.html", {"username": request.user.student.first_name}
        )
...
def test_view(request, topic_id, subtopic_id):
    ...
    if not student.can_take_experimental_test:
        ...
        if not module:
            return HttpResponse(
                "No associated module found for this topic.", status=400
            )

        # Fetch the student's progress for this subtopic (record must already exist)
        student_progress = Progress.objects.filter(
            student=student, current_subtopic=subtopic
        ).first()

        if not student_progress:
            return HttpResponse(
                "Progress record not found. Ensure progress is created in the module view.",
                status=400,
            )
        ...
        questions = Question.objects.filter(
            topic=topic, subtopic=subtopic, assigned_at=assigned_value
        )
        ...
        if request.method == "POST":
            ...
            try:
                ...
                subtopic = get_object_or_404(
                    Subtopic, subtopic_id=subtopic_id, topic=topic
                )
                ...
                TCE_Misunderstanding_path = os.path.join(
                    BASE_DIR, "my_app", "ML", "TCE_Misunderstanding.xlsx"
                )
                ...
                for key, value in request.POST.items():
                    if key.startswith("question_"):
                        ...
                        question_ids_list.append(
                            question.question_id - (first_question_id - 1)
                        )  # -91
                        # Check correctness
                ...
                for que in question_ids_list:
                    try:
                        ...
                        question_row = misconception_matrix[
                            misconception_matrix["item_id"] == item_id
                        ]
                        ...
                        prediction, proficiency_vector = predict(
                            MODEL,
                            student.student_id,
                            int(que),
                            knowledge_vector,
                        ...
                        for idx in active_knowledge_indices:
                            ...
                            if prof_value > 0.6:
                                ...
                                if (
                                    correctness_list[question_ids_list.index(int(que))]
                                    == 0
                                ...
                                logger.info(
                                    f"No condition met for prof={prof_value:.4f}, idx {idx + 1}, Q{que}"
                                )

                    except Exception as e:
                        logger.error(f"Prediction error for Q{que}: {e}")

                weak_knowledge_indices = sorted(
                    [int(idx) + 1 for idx in weak_knowledge_indices]
                )
                ...
                try:
                        ...
                        if video.subtopic == subtopic:
                            try:
                                ...
                                video_misconceptions = [
                                    int(m.strip())
                                    for m in video.misconceptions.split(",")
                                    if m.strip().isdigit()
                                ]

                                # Get intersection between video misconceptions and weak knowledge indices
                                matched_misconceptions = list(
                                    set(video_misconceptions)
                                    & set(weak_knowledge_indices)
                                )
                                print(
                                    "matched_misconceptions before DB - ",
                                    matched_misconceptions,
                                )

                                if matched_misconceptions:
                                    all_matched_misconception_ids.update(
                                        matched_misconceptions
                                    )
                                    ...
                                    if created:
                                        ...
                                        logger.info(
                                            "  → VideoProgress already exists. Skipping creation."
                                        )

                            except Exception as inner_e:
                                logger.error(
                                    f"Error processing video ID {video.video_module_id if video else 'Unknown'}: {inner_e}"
                                )
                    ...
                    if all_matched_misconception_ids:
                        ...
                        misconceptions = Misconception.objects.filter(
                            misconception_id__in=all_matched_misconception_ids
                        )
                    ...
                    logger.error(
                        "Error while assigning videos based on misconceptions:", e
                    )
                ...
                return redirect(
                    "test_results", topic_id=topic_id, subtopic_id=subtopic_id
                )
...
def modules_view(request):
    ...
    if not student.can_take_experimental_test:
        ...
        for topic in topics:
            completed_subtopics = list(
                Progress.objects.filter(
                    student=student, current_topic=topic, completion_status="Completed"
                ).values_list("current_subtopic__subtopic_name", flat=True)
            )

            progress = (
                Progress.objects.filter(student=student, current_topic=topic)
            ...
            if progress:
                    ...
                    if next_subtopic:
                        ...
                        if not next_progress:
                            new_progress = Progress.objects.create(
                                ...
                                .order_by("subtopic_order_number")
                                .first(),
                                module=VideoModule.objects.filter(
                        ...
                        upcoming_subtopics = [progress.current_subtopic] + list(
                            Subtopic.objects.filter(
                                topic=topic,
                ...
                upcoming_subtopics = (
                    list(
                        Subtopic.objects.filter(topic=topic).order_by(
                            "subtopic_order_number"
                        )
                    )
                    if first_subtopic
                    else []
...
def test_results(request, topic_id, subtopic_id):
    ...
    if not student.can_take_experimental_test:
        try:
            ...
            assessment = (
                Assessment.objects.filter(student=student, topic=topic)
                .order_by("-date_taken")
                .first()
            )
            if not assessment:
                return JsonResponse(
                    {"error": "No assessment data found for this topic."}, status=404
                )
            ...
            video_progress_entries = VideoProgress.objects.filter(
                student=student, subtopic=subtopic
            )
            video_data = []
            video_url = None
            progress = Progress.objects.filter(
            ...
            for entry in video_progress_entries:
                if not entry.watched:
                    ...
                    video_url = (
                        video_module.url
                        if video_module and progress and not progress.score_after
                        else None
                    )
...
def video_module_view(request):
    ...
    if not student.can_take_experimental_test:
        ...
        context = {"video_modules": video_modules}
...
def profile_update_view(request):
    ...
    if request.method == "POST":
        ...
        if password:
            ...
            messages.success(
                request, "Your password has been updated. Please log in again."
            )
...
def student_assignments_view(request):
    ...
    if not student.can_take_experimental_test:
        progress_data = (
            ...
            .select_related("current_subtopic__topic")
            .order_by("-last_accessed")
        )
        ...
        for p in sorted_progress_data:
            if p.current_subtopic:
                ...
                grouped_data[topic]["subtopics"].append(
                    p.current_subtopic.subtopic_name
                )
                grouped_data[topic]["before"].append(
                    float(p.score_before) if p.score_before is not None else None
                )
                grouped_data[topic]["after"].append(
                    float(p.score_after) if p.score_after is not None else None
                )
        ...
        if request.method == "POST":
            try:
                ...
                video = get_object_or_404(
                    VideoModule, video_module_id=video_id, subtopic=subtopic
                )
                ...
                try:
                    ...
                    raise Exception(
                        f"VideoProgress entry not found for student {student.student_id}, video {video.video_module_id}, subtopic {subtopic.subtopic_id}"
                    )
                ...
                if not video_progress.watched:
                    ...
                    total_videos = VideoProgress.objects.filter(
                        subtopic=subtopic, student=student
                    ).count()
                    ...
                    if watched_videos == total_videos:
                        progress = student.progress_set.filter(
                        ...
                        if progress:
                            ...
                            progress.save()
                            logger.info(
                                f"DEBUG: All videos watched for subtopic {subtopic_id}. Progress updated."
                            )
                        else:
                            logger.info(
                                f"DEBUG: No progress record found for subtopic {subtopic_id} for student {student}."
                            )

                    return JsonResponse(
                        {"success": True, "message": "Video marked as watched"}
                    )
                else:
                    return JsonResponse(
                        {"success": False, "message": "Video already watched"}
                    )
...
def experiment_home(request):
    ...
    context = {"username": student.first_name}
...
def display_experiment_test(request):
    ...
    # print(question_data)
    context = {"question_data": question_data, "username": student.first_name}
...
def submit_experimental_test(request):
    ...
    ExperimentAssessmentScore.objects.create(student=student, score=score_percentage)

    # Store data in session
    request.session["experiment_score"] = round(score_percentage, 2)
    request.session["experiment_responses"] = response_data
    request.session["student_name"] = (
        request.user.get_full_name() or request.user.username
    )

    messages.success(
        request, f"Test submitted successfully! Score: {total_score}/{total_questions}"
    )
...
def all_learning_videos(request):
    ...
    try:
        ...
        context = {"video_data_grouped": video_data_grouped}
...
_OPENAI_MODEL = os.environ.get("OPENAI_MODEL", "gpt-5.1")
_OPENAI_SERVICE_TIER = os.environ.get("OPENAI_SERVICE_TIER", "").strip() or None
...
def _call_openai(prompt: str) -> str:
    """Call the OpenAI chat completions API with optional service_tier."""
    kwargs = {
        "model": _OPENAI_MODEL,
        "messages": [
    ...
    if _OPENAI_SERVICE_TIER:
        kwargs["service_tier"] = _OPENAI_SERVICE_TIER
...
def ai_chat_helper(request):
    ...
    try:
        ...
        if not user_input or not selected_video_title:
            return JsonResponse(
                {"response": "Message and video title are required."}, status=400
            )
        ...
        prompt = (
            "Answer ONLY the user's question about the video below. "
        ...
        try:
            full_response = _call_openai(prompt)
            answer_part = full_response.split("\n\n")[0].strip()
            ...
            logger.error(f"OpenAI API error after retries: {str(api_err)}")
            return JsonResponse(
                {
                    "response": "The AI service encountered an error. Please try again later."
                },
                status=503,
            )
